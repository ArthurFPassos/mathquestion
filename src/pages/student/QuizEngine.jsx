...
function SortableItem({ id, label, disabled }) {
  ...
  const style = { transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.5 : 1, cursor: disabled ? "default" : "grab" };
  return (
      ...
      <div className="qe-match-grid">
        <div className="qe-match-col">
          {question.pairs.map((pair, idx) => (
            <div key={idx} className={`qe-match-item qe-match-item--left${selected?.side==="left"&&selected.index===idx?" qe-match-item--selected":""}${connections[idx]!==undefined?" qe-match-item--connected":""}`} onClick={() => handleLeft(idx)}>
        ...
        <div className="qe-match-lines">
          {question.pairs.map((_, i) => (
            <div key={i} className="qe-match-line-slot">{connections[i]!==undefined && <div className="qe-match-line-badge">↔</div>}</div>
          ))}
        </div>
        <div className="qe-match-col">
          {shuffledRight.map((item, idx) => (
            <div key={idx} className={`qe-match-item qe-match-item--right${selected?.side==="right"&&selected.index===idx?" qe-match-item--selected":""}${Object.values(connections).includes(idx)?" qe-match-item--connected":""}`} onClick={() => handleRight(idx)}>
              {item.label}
            </div>
          ))}
        </div>
...
// ── RF22: Gera PDF detalhado do módulo ────────────────────────────────────────
...
async function downloadModuleReportPDF(report) {
  ...
  // ── Seção de rascunhos ─────────────────────────────────────────────────────
  const questionsWithScratchpad = report.questions.filter((q) => q.scratchpadImage);
  if (questionsWithScratchpad.length > 0) {
    let curY = doc.lastAutoTable?.finalY + 12 || 200;
    ...
    for (const q of questionsWithScratchpad) {
      ...
      if (curY + 60 > pageH - 20) { doc.addPage(); curY = 20; }

      doc.setFontSize(8); doc.setFont("helvetica","bold"); doc.setTextColor(...DARK);
      doc.text(`Questão ${q.index+1}: ${q.statement.length>80?q.statement.slice(0,80)+"…":q.statement}`, 14, curY);
      curY += 5;

      try {
        // Insere o canvas como imagem PNG no PDF
        doc.addImage(q.scratchpadImage, "PNG", 14, curY, 120, 40);
        curY += 46;
      } catch (_) {
...
export default function QuizEngine() {
  ...
  // Ref para capturar snapshot do rascunho
  ...
  // Captura snapshot do rascunho e registra a questão no log
  const logAndAdvance = (correct, skipped = false) => {
    const scratchpadImage = scratchpadRef.current?.getSnapshot() || null;
    setQuestionLog((log) => [...log, {
      ...
      correct,
      skipped,
      attemptsCount:  attemptsCount + (skipped ? 0 : 1),
      hintUsed:       showHint,
      xpEarned:       correct ? currentXP : 0,
      scratchpadImage,                          // Base64 do canvas ou null
    }]);
    setAttemptsCount(0);
    ...
    if (isLastQ) {
      ...
      setShowHint(false); setInteractiveAnswered(false);
    }
  };

  const handleNext = () => logAndAdvance(true);
  const handleSkip = () => { setScores((s) => [...s, 0]); setXpLog((x) => [...x, 0]); logAndAdvance(false, true); };
  ...
  // ── Tela de resultado (RF22) ───────────────────────────────────────────────

  if (moduleReport) {
    const pct    = Math.round(moduleReport.score * 100);
    const passed = moduleReport.score >= 0.8;
    const withScratchpad = moduleReport.questions.filter((q) => q.scratchpadImage).length;
    return (
      <div className="qe-wrapper">
        <div className="qe-result-card">
          ...
          {withScratchpad > 0 && (
            <p className="qe-result-scratchpad-note">✏️ {withScratchpad} rascunho{withScratchpad>1?"s":""} incluído{withScratchpad>1?"s":""} no PDF</p>
          )}
  ...
  return (
    <div className="qe-wrapper">
      ...
      {state.scratchpadOpen && <Scratchpad ref={scratchpadRef} />}
          ...
          <>
            ...
            <div className="qe-error-panel">
              <button className="qe-error-opt-btn" onClick={handleRetry}>
                ...
                <div className="qe-error-opt-text"><span className="qe-error-opt-title">Tentar novamente</span><span className="qe-error-opt-sub">Sem usar dica</span></div>
              </button>
              <button className="qe-error-opt-btn qe-error-opt-btn--hint" onClick={handleUseHint} disabled={!canShowHint}>
                ...
                <div className="qe-error-opt-text">
                  ...
                  <span className="qe-error-opt-sub">{canShowHint?`−1 do contador (${hintsLeft} restante${hintsLeft!==1?"s":""})`:showHint?"Dica já utilizada":"Sem dicas disponíveis"}</span>
                </div>
              </button>
              <button className="qe-error-opt-btn qe-error-opt-btn--skip" onClick={handleSkip}>
                <span className="qe-error-opt-icon">⏭</span>
                <div className="qe-error-opt-text"><span className="qe-error-opt-title qe-error-opt-title--danger">Pular questão</span><span className="qe-error-opt-sub">Registra como errada (0 XP)</span></div>
              </button>
            </div>
          </>
        )}
